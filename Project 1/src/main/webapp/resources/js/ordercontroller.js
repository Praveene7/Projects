...
cartApp.controller("addToCartCtrl", function ($scope, $http){
    ...
    $scope.addItemToCart = function (id) {
            $http.put('http://localhost:8080/ShopCart/usercart/cart/addItem/'+id).success(function () {
    ...
    $scope.clearCartItems = function () {
    	$http['delete']('http://localhost:8080/ShopCart/usercart/cart/clearCartItems/'+$scope.cartId).success(function (data) {$scope.refreshCartItems()});
    };
});